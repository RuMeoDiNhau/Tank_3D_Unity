...
namespace Tanks.Complete
{
    ...
    public class GameUIHandler : MonoBehaviour
    {
        ...
        {
            if (m_StartMenuRoot != null)
                m_StartMenuRoot.gameObject.SetActive(visible);
        }

        public void CleanupMenuPreviews()
        {
            ...
            // Safety net: destroy any leftover preview tanks parented under slots.
            foreach (var slot in m_PlayerSlots)
            {
                if (slot == null)
                    continue;

                // Previews are parented under the slot transform (see StartMenuSlot.SetTankPreview).
                // Only remove children that look like tank prefabs (have TankMovement).
                var childMovements = slot.GetComponentsInChildren<TankMovement>(includeInactive: true);
                foreach (var movement in childMovements)
                {
                    if (movement != null && movement.gameObject != slot.gameObject)
        ...
        private void Start()
        {
            ...
            {
                Debug.LogWarning(
                    $"[{nameof(GameUIHandler)}] Start Button is not assigned on '{name}'. Disabling GameUIHandler.");
                enabled = false;
                return;
            }
