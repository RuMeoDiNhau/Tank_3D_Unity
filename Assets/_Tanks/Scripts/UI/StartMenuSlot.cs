...
namespace Tanks.Complete
{
    ...
    public class StartMenuSlot : MonoBehaviour
    {
        ...
        public void SetTankPreview(GameObject prefab)
        {
            ...
            var movePrefab = prefab.GetComponent<TankMovement>();
            var shootPrefab = prefab.GetComponent<TankShooting>();
            var healthPrefab = prefab.GetComponent<TankHealth>();

            string speedStr = movePrefab != null ? movePrefab.m_Speed.ToString() : "N/A";
            string damageStr = shootPrefab != null ? shootPrefab.m_MaxDamage.ToString() : "N/A";
            string healthStr = healthPrefab != null ? healthPrefab.m_StartingHealth.ToString() : "N/A";

            // Cập nhật text UI
            m_TankStats.text = $"Speed {speedStr}\nDamage {damageStr}\nHealth: {healthStr}";

            
            var networkBehaviours = TankPreview.GetComponentsInChildren<NetworkBehaviour>(true);
            ...
            var networkObjects = TankPreview.GetComponentsInChildren<NetworkObject>(true);
            ...
            TankPreview.transform.SetParent(transform, worldPositionStays: true);
            
            
            var moveInst = TankPreview.GetComponent<TankMovement>();
            if(moveInst != null) moveInst.enabled = false;
