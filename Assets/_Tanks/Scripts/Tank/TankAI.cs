...
namespace Tanks.Complete
{
    ...
    private void Start()
    {
        // Lệnh này chạy khi tất cả xe tăng đã được GameManager đẻ ra xong
        m_AllTanks = FindObjectsByType<TankMovement>(FindObjectsInactive.Include, FindObjectsSortMode.None).Select(e => e.gameObject).ToArray();
    }
        ...
        void SeekUpdate()
        {
            ...
            if (m_PathfindTimer > m_PathfindTime)
            {
                ...
                // Fallback target when we can't find any NavMesh path (e.g. no baked NavMesh, or tank not on NavMesh).
                // In that case we still want to move directly toward the closest active tank.
                Transform closestTarget = null;
                float closestTargetSqrDist = float.MaxValue;
                ...
                for (var i = 0; i < m_AllTanks.Length; i++)
                {
                    ...
                    var toTank = tank.transform.position - transform.position;
                    toTank.y = 0;
                    float sqrDist = toTank.sqrMagnitude;
                    if (sqrDist < closestTargetSqrDist)
                    {
                        closestTargetSqrDist = sqrDist;
                ...
                // usedPath will still be -1 if the tank could not find a path to any tank.
                // In that case we still keep the closest target and move directly toward it as a fallback,
                // so AI tanks can work even when the scene has no baked NavMesh.
                if (usedPath == -1 && closestTarget != null)
                {
        ...
        private void StartFleeing()
        {
            ...
            {
                // No path found, go back to seek mode and move directly toward the flee point
                m_CurrentState = State.Seek;
                m_CurrentPath = null;
                m_IsMoving = true;
            }
        ...
        {
            // If we do not have either a path or a target, exit early.
            if (m_CurrentTarget == null && (m_CurrentPath == null || m_CurrentPath.corners.Length == 0))
                return;
            
            var rb = m_Movement.Rigidbody;
            
            // The point we will orient toward. By default, the current corner in our path.
            // If no path exists, we steer directly toward the current target.
            Vector3 orientTarget = m_CurrentTarget.position;
            if (m_CurrentPath != null && m_CurrentPath.corners.Length > 0)
                orientTarget = m_CurrentPath.corners[Mathf.Min(m_CurrentCorner, m_CurrentPath.corners.Length - 1)];
