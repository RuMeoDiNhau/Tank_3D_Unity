...
namespace Tanks.Complete
{
    public class TankShooting : NetworkBehaviour
    {
        ...
        // Đã đổi tên biến để không bị trùng với thuộc tính NetworkObject có sẵn của NetworkBehaviour
        private NetworkObject m_TankNetworkObject;
        ...
        private void Awake()
        {
            m_TankNetworkObject = GetComponent<NetworkObject>();
        ...
        {
            // When Netcode is not running, always allow local control (offline mode).
            if (NetworkManager.Singleton == null || !NetworkManager.Singleton.IsListening)
                return true;

            if (m_TankNetworkObject == null)
                return true;

            if (!m_TankNetworkObject.IsSpawned)
                return true;

            return m_TankNetworkObject.IsOwner;
        }
        ...
        private void Fire ()
        {
            // Online mode: spawn shells on the server so damage and HP stay in sync.
            if (NetworkManager.Singleton != null && NetworkManager.Singleton.IsListening &&
                m_TankNetworkObject != null && m_TankNetworkObject.IsSpawned)
            {
                if (!m_TankNetworkObject.IsOwner)
                    return;

                // Set the fired flag so Fire is only called once.
                m_Fired = true;

                float shotMaxDamage = m_MaxDamage;

                // If extra damage PowerUp is active, apply it to this shot and clear local UI/state.
                if (m_HasSpecialShell)
                {
                    shotMaxDamage *= m_SpecialShellMultiplier;
                    m_HasSpecialShell = false;
                ...
                // Play fire audio locally for responsiveness.
                if (m_ShootingAudio == null)
                    m_ShootingAudio = GetComponent<AudioSource>();
                if (m_ShootingAudio != null)
                {
                ...
                Vector3 pos = m_FireTransform.position;
                Quaternion rot = m_FireTransform.rotation;
                float launchForce = m_CurrentLaunchForce;

                if (IsServer)
                    SpawnShellServer(pos, rot, launchForce, shotMaxDamage);
                else
                    FireServerRpc(pos, rot, launchForce, shotMaxDamage);

                // Reset launch force and apply cooldown locally.
                m_CurrentLaunchForce = m_MinLaunchForce;
        ...
        private void FireServerRpc(Vector3 position, Quaternion rotation, float launchForce, float shotMaxDamage)
        {
            SpawnShellServer(position, rotation, launchForce, shotMaxDamage);
        }

        private void SpawnShellServer(Vector3 position, Quaternion rotation, float launchForce, float shotMaxDamage)
        {
            if (!IsServer)
                return;
            ...
            if (m_Shell == null)
            {
                Debug.LogError("[TankShooting] Shell prefab is not assigned.");
                return;
            }

            var shellGo = Instantiate(m_Shell.gameObject, position, rotation);

            var rb = shellGo.GetComponent<Rigidbody>();
            if (rb != null)
                rb.linearVelocity = launchForce * shellGo.transform.forward;
            ...
            if (explosionData != null)
            {
                explosionData.m_ExplosionForce = m_ExplosionForce;
                explosionData.m_ExplosionRadius = m_ExplosionRadius;
                explosionData.m_MaxDamage = shotMaxDamage;
            }

            var netObj = shellGo.GetComponent<NetworkObject>();
            if (netObj == null)
            {
                // Auto-add NetworkObject if missing (for multiplayer sync)
                netObj = shellGo.AddComponent<NetworkObject>();
                shellGo.AddComponent<NetworkTransform>(); // Default sync settings are sufficient for shells
            }
